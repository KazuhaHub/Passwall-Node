...
func (c *helperController) rollback(receipt Receipt, backup helperBackup, uid, gid uint32, message string) error {
	...
	if err := c.restoreBackup(receipt.Request.Task.ID, backup, gid); err != nil {
		...
		return c.markIndeterminate(receipt, gid,
			c.restartAfterFailedRestore(receipt, backup, uid, "retained previous files could not be restored"))
	}
	if _, err := c.command(ctx, "start", nodeService); err != nil {
...
func (c *helperController) restartAfterFailedRestore(receipt Receipt, backup helperBackup, uid uint32, message string) string {
	// A FRESH BUDGET, NOT ROLLBACK'S. That one has already carried a stop — which
	// systemd lets run to TimeoutStopSec, 90 seconds by default — and then a
	// restore that failed slowly enough to matter. The single start that decides
	// whether this node serves traffic must not inherit whatever is left of it.
	...
	case backup.OldSHA256:
		installed, expected = "the retained previous release", backup.PreviousVersion
	case backup.NewSHA256:
		installed = "the verified target release"
		if receipt.Result != nil {
			expected = receipt.Result.Version
		}
	default:
	...
	// THE BINARY IS NOT THE WHOLE INSTALLATION. restoreBackup rewrites four managed
	// files in order, the executable first, so a failure between them leaves the
	// binary and the recorded version disagreeing. run() refuses a LATER upgrade on
	// exactly that comparison — and reports it as the schema contract, which does
	// not describe what happened. Bringing the node back up makes that mismatch
	// quiet, so it has to be named here or nobody will find it.
	mixed := ""
	if recorded, versionErr := readManagedVersion(c.root); versionErr != nil {
		mixed = "; the recorded version could not be read, so the installation is inconsistent and the next upgrade will be refused"
	} else if expected != "" && recorded != expected {
		mixed = "; the recorded version is " + recorded + " while the installed binary is " + expected +
			", so the installation is inconsistent and the next upgrade will be refused until it is repaired"
	}

	if _, err := c.command(ctx, "start", nodeService); err != nil {
		return message + "; " + installed + " is installed but the service could not be started" + mixed
	}
	// A START THAT RETURNED IS NOT A PROCESS THAT IS RUNNING. The success path
	// above confirms identity before it claims anything; this confirms it too when
	// the seam is available, because "was restarted" is what an operator will act
	// on, and in the target case the binary being started is the one that just
	// failed to prove readiness.
	if c.process != nil {
		if err := c.waitProcess(ctx, digest, uid); err != nil {
			return message + "; " + installed + " was started but its process could not be confirmed" + mixed
		}
	}
	return message + "; " + installed + " is installed and was restarted" + mixed
}
