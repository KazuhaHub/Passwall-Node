// Package nodefixture is shared by disposable-runner acceptance tests.
...
type Observation struct {
	...
	CurrentAcknowledged bool
	LatestCoreState     string
	Capabilities        []string
	TaskResults         map[string]protocol.TaskResult
	ResultVersions      map[string]string
}
...
func New(agentID, credential string) (*Fixture, error) {
	...
	f := &Fixture{agentID: agentID, credential: credential, tasks: make(map[string]protocol.Task)}
...
func (f *Fixture) ServeHTTP(w http.ResponseWriter, request *http.Request) {
	...
	f.observation.LatestVersion = report.AgentVersion
	f.observation.Capabilities = append([]string(nil), report.Capabilities...)
	for _, result := range report.TaskResults {
		if task, ok := f.tasks[result.ID]; ok && task.Kind == result.Kind && task.InputSHA256 == result.InputSHA256 && task.NotAfterMS == result.NotAfterMS {
			if f.observation.TaskResults == nil {
				f.observation.TaskResults = make(map[string]protocol.TaskResult)
				f.observation.ResultVersions = make(map[string]string)
			}
			result.Result = append([]byte(nil), result.Result...)
			f.observation.TaskResults[result.ID] = result
			f.observation.ResultVersions[result.ID] = report.AgentVersion
			delete(f.tasks, result.ID)
		}
	}
	...
	f.observation.CurrentAcknowledged = acknowledged
	f.observation.LatestCoreState = report.CoreState
	f.mu.Unlock()
	w.Header().Set("Content-Type", "application/json")
	_ = json.NewEncoder(w).Encode(response)
}

func (f *Fixture) Snapshot() Observation {
	f.mu.Lock()
	defer f.mu.Unlock()
	observation := f.observation
	observation.Capabilities = append([]string(nil), observation.Capabilities...)
	observation.TaskResults = make(map[string]protocol.TaskResult)
	observation.ResultVersions = make(map[string]string)
	for id, result := range f.observation.TaskResults {
		result.Result = append([]byte(nil), result.Result...)
		observation.TaskResults[id] = result
		observation.ResultVersions[id] = f.observation.ResultVersions[id]
	}
	return observation
}

func (f *Fixture) BeginReinstall() {
	f.mu.Lock()
...
func Start(f *Fixture) (*http.Server, string, []byte, error) {
	...
	return server, "https://" + listener.Addr().String() + Path, certPEM, nil
}

func (f *Fixture) QueueTask(task protocol.Task) error {
	if task.NotAfterMS <= 0 || protocol.ValidateTasks([]protocol.Task{task}) != nil {
		return errors.New("fixture task must have a valid immutable identity and deadline")
	}
	f.mu.Lock()
	defer f.mu.Unlock()
	for _, required := range []string{protocol.CapabilityTaskExecutionV1, protocol.CapabilityTaskExpiryV1, protocol.TaskCapability(task.Kind)} {
		found := false
		for _, capability := range f.observation.Capabilities {
			if capability == required {
				found = true
			}
		}
		if !found {
			return errors.New("fixture must observe every negotiated task capability before dispatch")
		}
	}
	if _, exists := f.tasks[task.ID]; exists {
		return errors.New("fixture task identity is already queued")
	}
	task.Args = append([]byte(nil), task.Args...)
