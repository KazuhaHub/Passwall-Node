...
const repository = 'kazuhahub/passwall-node'
const tag = 'v0.0.1-beta2'
...
test('both version shapes reach the registry', async () => {
  for (const version of ['4.0.0', 'v0.0.1-beta2', '102.1.0', 'v0.0.1-beta12']) {
...
test('invalid repository/tag coordinates are rejected before any network request', async () => {
  for (const [candidateRepository, candidateTag] of [
    ['KazuhaHub/passwall-node', tag], ['kazuhahub/foreign-package', tag],
    ['kazuhahub/passwall-node/extra', tag], ['kazuhahub@evil.test/passwall-node', tag],
    ['../passwall-node', tag], ['owner_name/passwall-node', tag], ['a'.repeat(40) + '/passwall-node', tag],
    ...
    // THE PRODUCT SHAPE IS NOT A LICENCE TO PASS ANYTHING. These are the
    // same refusals in the shape that used to be turned away by the missing
    // `v`, and they are why the leading character and the reserved names are
