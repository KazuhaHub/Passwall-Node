...
**安装反馈简化（开发完成，未发布，2026-09-12）**：私有 Linux／systemd 安装器按六阶段输出
...
配套 PSP 功能分支仍正式依赖已发布 beta3；此安装器需先随新 PN 发行并通过核验，再由 PSP 更新依赖。
本地隔离 shell／PTY、全量 Go 与 race 测试已通过；未操作线上，也没有宣称本次真实 systemd 通过。
